...
public class CircleImageView extends ImageView {
    ...
    private void updateBitmapSize() {
        ...
        if (mBitmap.getWidth() < mBitmap.getHeight()) {
            ...
            dy = mBitmapDrawBounds.top - (mBitmap.getHeight() * scale / 2f) + (mBitmapDrawBounds.width()
                    / 2f);
        } else {
            scale = mBitmapDrawBounds.height() / (float) mBitmap.getHeight();
            dx = mBitmapDrawBounds.left - (mBitmap.getWidth() * scale / 2f) + (mBitmapDrawBounds.width()
                    / 2f);
